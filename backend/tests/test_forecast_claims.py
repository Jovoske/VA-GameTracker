...
from app.forecasting.model import MIN_NIGHTS_TO_JUDGE, _camera_nights, _verdict, forecast_tonight
...
def test_thin_data_is_no_data_not_quiet():
    ...
    assert _verdict(0.9, camera_nights=3) == "NO_DATA"
    assert _verdict(0.01, camera_nights=3) == "NO_DATA"
    assert _verdict(0.9, camera_nights=MIN_NIGHTS_TO_JUDGE) == "BEST_ODDS"
...
def test_late_deployed_camera_is_not_capped_by_estate_history(db_session):
    ...
    assert _camera_nights(db_session, old_cam.id) == 40
    assert _camera_nights(db_session, new_cam.id) == 10
...
def test_payload_no_longer_carries_a_fabricated_confidence(db_session, monkeypatch):
    ...
    assert rec["camera_nights"] == 20
    assert rec["nights_present"] == 10
    assert "10 of the 20 nights this camera was up" in rec["reason"]
...
def test_outlook_makes_no_per_night_prediction(db_session, monkeypatch):
    ...
    rows = _outlook(db_session, {"recommended": None})
    assert len(rows) == 7
    for row in rows:
